...
export default function Home() {
  ...
  function prevPremiere() {
    if (currentPremiere > 1) setCurrentPremiere(currentPremiere - 1);
    ...
    <div>
      ...
      <div className={s.contenedorcartelera}>
        ...
        <div className={s.cartelera}>
          ...
          {carteleraActual.length === 0 && (
            <div>
              <h2>
                El título que estás buscando no se encuentra disponible en este
                momento. Prueba corrigiendo tu búsqueda o inténtalo de nuevo más
                tarde.
              </h2>
              <br />
              <button
                class="btn btn-warning"
                onClick={(event) => handleVolverBtn(event)}
              >
                Volver
              </button>
            </div>
      ...
      <div className={s.cartelera}>
        ...
        {premiereActual?.map((c) => {
          return (
            <div className={s.card}>
              <Link to={"/movies/" + c.id}>
                <Card Poster={c.Poster} Title={c.Title} Release={c.Release} />
              </Link>
            </div>
          );
        })}
        ...
        <div className={s.contenedorpag}>
          {counterPremiere < premiere.length - 4 && (
            <i
              class="bi bi-chevron-right"
              style={{ fontSize: "32px" }}
