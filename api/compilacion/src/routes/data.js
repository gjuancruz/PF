"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.SupermanSpidermanorBatman = exports.MinionsHolidaySpecial = exports.Minions = exports.DrStranger = exports.topGun = exports.lightyear = exports.MinionsTheRiseofGru = exports.jurassic = exports.thor = exports.iceAge = exports.sonic = exports.spider = exports.cars = void 0;
exports.cars = {
    Title: "Cars",
    Plot: "El aspirante a campeón de carreras Rayo McQueen parece que está a punto de conseguir el éxito. Su actitud arrogante se desvanece cuando llega a una pequeña comunidad olvidada que le enseña las cosas importantes de la vida que había olvidado",
    Genre: "Animación, Aventura, Comedia",
    Actors: "Owen Wilson, Bonnie Hunt, Paul Newman",
    Language: "Español, Inglés, Italiano, Japones",
    Director: "John Lasseter, Joe Ranft",
    Release: "09 Jun 2006",
    Poster: "https://m.media-amazon.com/images/M/MV5BMTg5NzY0MzA2MV5BMl5BanBnXkFtZTYwNDc3NTc2._V1_SX300.jpg",
    Rated: "G",
    Trailer: "https://youtu.be/nuQDFYpPUh4",
    Type: "3D",
    Runtime: 117
};
exports.spider = {
    Title: "Spider-Man: Sin Camino a Casa",
    Plot: "Tras descubrirse la identidad secreta de Peter Parker como Spider-Man, la vida del joven se vuelve una locura. Peter decide pedirle ayuda al Doctor Extraño para recuperar su vida. Pero algo sale mal y provoca una fractura en el multiverso",
    Genre: "Acción, Aventura, Fantasía",
    Actors: "Tom Holland, Zendaya, Benedict Cumberbatch",
    Language: "Español, Inglés",
    Director: "Jon Watts",
    Release: "17 Dec 2021",
    Poster: "https://m.media-amazon.com/images/M/MV5BZWMyYzFjYTYtNTRjYi00OGExLWE2YzgtOGRmYjAxZTU3NzBiXkEyXkFqcGdeQXVyMzQ0MzA0NTM@._V1_SX300.jpg",
    Rated: "PG-13",
    Trailer: "https://youtu.be/8nNa5A_MRBU",
    Type: "2D",
    Runtime: 148
};
exports.sonic = {
    Title: "Sonic 2",
    Plot: "Después de establecerse en Green Hills, Sonic quiere demostrar que tiene madera de héroe. La prueba de fuego llega con el retorno del malvado Robotnik, y su nuevo compinche, Knuckles, en busca de una esmeralda que destruye civilizaciones",
    Genre: "Acción, Aventura, Comedia",
    Actors: "James Marsden, Jim Carrey, Ben Schwartz",
    Language: "Español, Inglés, Ruso",
    Director: "Jeff Fowler",
    Release: "08 Apr 2022",
    Poster: "https://m.media-amazon.com/images/M/MV5BMGI1NjA1MjUtNGQxNC00NDYyLThjODgtZjFkZjQ4OGM0NDc5XkEyXkFqcGdeQXVyMTM0NTUzNDIy._V1_SX300.jpg",
    Rated: "PG",
    Type: "3D",
    Trailer: "https://youtu.be/2OMixTIRQcY",
    Runtime: 122
};
exports.iceAge = {
    Title: "Ice Age",
    Plot: "Hace 20.000 años, la Tierra es un lugar prehistórico lleno de peligros, entre ellos el comienzo de la Edad de hielo. Para evitar quedarse congelados, las criaturas majestuosas empiezan a migrar hacia el sur. Entonces, un grupo de animales muy distintos, entre los que se encuentra un mamut gruñón y un perezoso, deben ayudar a un niño perdido a regresar a su clan",
    Genre: "Animación, Aventura, Comedia",
    Actors: "Denis Leary, John Leguizamo, Ray Romano",
    Language: "Español, Inglés",
    Director: "Chris Wedge, Carlos Saldanha",
    Release: "15 Mar 2002",
    Poster: "https://m.media-amazon.com/images/M/MV5BOGEwMTQyMDktMWUwZC00MzExLTg1MGMtYWJiNWNhMzIyMGU5XkEyXkFqcGdeQXVyOTYyMTY2NzQ@._V1_SX300.jpg",
    Rated: "PG",
    Type: "2D",
    Trailer: "https://youtu.be/RI7e1IksKEQ",
    Runtime: 81
};
exports.thor = {
    Title: "Thor: Amor y Trueno",
    Plot: "Thor emprende un viaje que no se parece en nada a lo que se ha enfrentado hasta ahora: una búsqueda de la paz interior. Pero el retiro de Thor se ve interrumpido por un asesino galáctico conocido como Gorr el Carnicero de Dioses, que busca la extinción de los dioses. Para hacer frente a la amenaza, Thor solicita la ayuda del Rey Valkiria, de Korg y de su ex novia Jane Foster que, para sorpresa de Thor, empuña inexplicablemente su martillo mágico, Mjolnir, como la Poderosa Thor",
    Genre: "Acción, Aventura, Comedia",
    Actors: "Chris Hemsworth, Natalie Portman, Christian Bale",
    Language: "Español, Inglés",
    Director: "Taika Waititix",
    Release: "08 Jul 2022",
    Poster: "https://m.media-amazon.com/images/M/MV5BYmMxZWRiMTgtZjM0Ny00NDQxLWIxYWQtZDdlNDNkOTEzYTdlXkEyXkFqcGdeQXVyMTkxNjUyNQ@@._V1_SX300.jpg",
    Rated: "PG-13",
    Type: "3D",
    Trailer: "https://youtu.be/E5xC9i_1vvY",
    Runtime: 125
};
exports.jurassic = {
    Title: "Jurassic World Dominio",
    Plot: "La isla Nublar y su parque han sido destruidos, pero el problema no se terminó. Con los dinosaurios dispersos por todo el mundo, la convivencia entre el presente y el pasado alcanza un nuevo nivel de tensión",
    Genre: "Acción, Aventura, Ciencia Ficción",
    Actors: "Chris Pratt, Bryce Dallas Howard, Laura Dern",
    Language: "Español, Inglés",
    Director: "Colin Trevorrow",
    Release: "10 Jun 2022",
    Poster: "https://m.media-amazon.com/images/M/MV5BOTBjMjA4NmYtN2RjMi00YWZlLTliYTktOTIwMmNkYjYxYmE1XkEyXkFqcGdeQXVyODE5NzE3OTE@._V1_SX300.jpg",
    Rated: "PG-13",
    Type: "2D",
    Trailer: "https://youtu.be/rjQDdt5O99U",
    Runtime: 147
};
exports.MinionsTheRiseofGru = {
    Title: "Minions: Nace un Villano",
    Plot: "En los años 70, Gru crece siendo un gran admirador de Los salvajes seis, un supergrupo de villanos. Para demostrarles que puede ser malvado, Gru idea un plan con la esperanza de formar parte de la banda. Por suerte, cuenta con la ayuda de sus fieles seguidores, los Minions, siempre dispuestos a sembrar el caos",
    Genre: "Animación, Aventura, Comedia",
    Actors: "Steve Carell, Pierre Coffin, Taraji P. Henson",
    Language: "Español, Inglés",
    Director: "Kyle Balda, Brad Ableson",
    Release: "13 Jul 2022",
    Poster: "https://m.media-amazon.com/images/M/MV5BZDQyODUwM2MtNzA0YS00ZjdmLTgzMjItZWRjN2YyYWE5ZTNjXkEyXkFqcGdeQXVyMTI2MzY1MjM1._V1_SX300.jpg",
    Rated: "PG",
    Type: "3D",
    Trailer: "https://youtu.be/tOCczL-LYHo",
    Runtime: 87
};
exports.lightyear = {
    Title: "Lightyear",
    Plot: "Buzz Lightyear se embarca en una aventura intergaláctica con un grupo de reclutas ambiciosos y su compañero robot. Los amigos tendrán que aprender a trabajar juntos para escapar del malvado Zurg y su ejército de robots.",
    Genre: "Animación, Acción, Aventura",
    Actors: "Chris Evans, Keke Palmer, Peter Sohn",
    Language: "Español, Inglés",
    Director: "Angus MacLane",
    Release: "30 Jul 2022",
    Poster: "https://m.media-amazon.com/images/M/MV5BYTg2Zjk0ZTctM2ZmMi00MDRmLWJjOGYtNWM0YjBmZTBjMjRkXkEyXkFqcGdeQXVyMTM1MTE1NDMx._V1_SX300.jpg",
    Rated: "PG",
    Type: "2D",
    Trailer: "https://youtu.be/lKLqrpYNfnA",
    Runtime: 100
};
exports.topGun = {
    Title: "Top Gun: Maverick",
    Plot: "El joven piloto Maverick Mitchell acude a una prestigiosa escuela aérea, famosa por formar a los mejores pilotos de combate del país. Maverick se siente atraído por su hermosa instructora, mientras desarrolla una intensa rivalidad con otro piloto.",
    Genre: "Action, Drama",
    Actors: "Tom Cruise, Jennifer Connelly, Miles Teller",
    Language: "Español, Inglés",
    Director: "Joseph Kosinski",
    Release: "27 Jul 2022",
    Poster: "https://m.media-amazon.com/images/M/MV5BOWQwOTA1ZDQtNzk3Yi00ZmVmLWFiZGYtNjdjNThiYjJhNzRjXkEyXkFqcGdeQXVyODE5NzE3OTE@._V1_SX300.jpg",
    Rated: "PG-13",
    Type: "2D",
    Trailer: "https://youtu.be/S_QkFkOb_HE",
    Runtime: 130
};
exports.DrStranger = {
    Title: "Doctor Strange en el Multiverso de la Locura",
    Plot: "El Dr. Stephen Strange abre un portal al multiverso al utilizar un hechizo prohibido. Ahora, su equipo debe enfrentarse a una amenaza que podría destruirlo todo.",
    Genre: "Acción, Aventura, Fantasía",
    Actors: "Benedict Cumberbatch, Elizabeth Olsen, Chiwetel Ejiofor",
    Language: "Español, Inglés",
    Director: "Sam Raimi",
    Release: "30 Jul 2022",
    Poster: "https://m.media-amazon.com/images/M/MV5BNWM0ZGJlMzMtZmYwMi00NzI3LTgzMzMtNjMzNjliNDRmZmFlXkEyXkFqcGdeQXVyMTM1MTE1NDMx._V1_SX300.jpg",
    Rated: "PG-13",
    Type: "3D",
    Trailer: "https://youtu.be/D9AzGVmMVpk",
    Runtime: 126
};
exports.Minions = {
    Title: "Minions",
    Plot: "Los minions, ingenuos y torpes ayudantes de villano, llevan buscando, desde el principio de los tiempos, un verdadero malhechor al que servir. A lo largo de una evolución de millones de años, los minions se han puesto al servicio de los amos más despreciables. Kevin, acompañado por el rebelde Stuart y el adorable Bob, emprende un emocionante viaje para conseguir una jefa a quien servir, la terrible Scarlet Overkill.",
    Genre: "Animación, Aventura, Comedia",
    Actors: "Sandra Bullock, Jon Hamm, Michael Keaton",
    Language: "Español, Inglés",
    Director: "Kyle Balda, Pierre Coffin",
    Release: "20 Jul 2022",
    Poster: "https://m.media-amazon.com/images/M/MV5BMTg2MTMyMzU0M15BMl5BanBnXkFtZTgwOTU3ODk4NTE@._V1_SX300.jpg",
    Rated: "PG",
    Type: "2D",
    Trailer: "https://youtu.be/3RyJ127cJR8",
    Runtime: 91,
};
exports.MinionsHolidaySpecial = {
    Title: "Minions Holiday Special",
    Plot: "Illumination presenta cuatro nuevas aventuras en mini-películas junto con apariciones especiales como invitados de algunos de los queridos personajes de Illumination de sus franquicias La vida secreta de las mascotas y Sing.",
    Genre: "Animación, Comedia",
    Actors: "Carlos Alazraqui, Pierre Coffin, Brian T. Delaney",
    Language: "Español, Inglés",
    Director: "Brad Ableson",
    Release: "27 Jul 2022",
    Poster: "https://m.media-amazon.com/images/M/MV5BMjFjMzUwZjYtZTM2MC00NThiLTllN2UtNzhmMGE5YjkyYzllXkEyXkFqcGdeQXVyMzY3MDEzMTA@._V1_SX300.jpg",
    Rated: "TV-PG",
    Type: "2D",
    Trailer: "https://youtu.be/Uds1H3HS68I",
    Runtime: 21,
};
exports.SupermanSpidermanorBatman = {
    Title: "Superman, Spiderman o Batman",
    Plot: "Aron, un niño de cinco años de edad, desea poder salvar a su madre de una enfermedad cardiaca como el superhéroe de un libro de historietas cómicas.",
    Genre: "Drama",
    Actors: "Adriana Bailescu, Zsolt Bogdán, Ovidiu Crisan",
    Language: "Romanian",
    Director: "Tudor Giurgiu",
    Release: "29 Jul 2022",
    Poster: "https://m.media-amazon.com/images/M/MV5BMjQ4MzcxNDU3N15BMl5BanBnXkFtZTgwOTE1MzMxNzE@._V1_SX300.jpg",
    Rated: "Not Rated",
    Type: "2D",
    Trailer: "https://youtu.be/vlzW65U5IAo",
    Runtime: 11,
};
//# sourceMappingURL=data.js.map